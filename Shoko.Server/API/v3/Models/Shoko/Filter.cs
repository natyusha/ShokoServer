...
public class Filter : BaseModel
{
    ...
    public Filter(HttpContext ctx, ShokoGroup_Filter groupFilter, bool fullModel = false)
    {
        ...
        Size = IsDirectory ? (
            RepoFactory.Shoko_Group_Filter.GetByParentID(groupFilter.GroupFilterID).Count
        ) : (
            groupFilter.GroupsIds.TryGetValue(user.Id, out var groupSet) ? groupSet.Count : 0
        );
    }
    ...
    /// <param name="gf"></param>
    /// <returns></returns>
    public static List<SortingCriteria> GetSortingCriteria(ShokoGroup_Filter gf)
    {
    ...
    public class FilterCondition
    {
        ...
        public FilterCondition(ShokoGroup_FilterCondition condition)
        {
    ...
    public class Input
    {
        ...
        public class CreateOrUpdateFilterBody
        {
            ...
            public CreateOrUpdateFilterBody(ShokoGroup_Filter groupFilter)
            {
                Name = groupFilter.GroupFilterName;
            ...
            public Filter? MergeWithExisting(HttpContext ctx, ShokoGroup_Filter groupFilter, ModelStateDictionary modelState, bool skipSave = false)
            {
                ...
                {
                    var parentFilter = RepoFactory.Shoko_Group_Filter.GetByID(ParentID.Value);
                ...
                {
                    var subFilters = groupFilter.GroupFilterID != 0 ? RepoFactory.Shoko_Group_Filter.GetByParentID(groupFilter.GroupFilterID) : new();
                ...
                else
                {
                    ...
                    if (Conditions != null)
                    {
                        groupFilter.Conditions = Conditions
                            .Select(c => new ShokoGroup_FilterCondition()
                            {
                                ConditionOperator = c.Operator,
                                ConditionParameter = c.Parameter ?? string.Empty,
