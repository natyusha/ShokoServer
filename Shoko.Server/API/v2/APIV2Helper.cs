...
public class APIV2Helper
{
    ...
    [Obsolete]
    public static string ConstructSearchUrl(HttpContext ctx, string limit, string query, bool searchTag,
        bool short_url = false)
    {
        if (searchTag)
        {
    ...
    private static string ConvertRestImageToNonRestUrl(HttpContext ctx, string url, bool short_url)
    {
        ...
        var link = url.ToLower();
        var split = link.Split(new[] { '/' }, StringSplitOptions.RemoveEmptyEntries);
        int type;
        if (int.TryParse(split[split.Length - 1], out var id)) // no ratio
        {
    ...
    public static Filter FilterFromGroupFilter(HttpContext ctx, GroupFilter gg, int uid)
    {
        var ob = new Filter
        {
            name = gg.GroupFilterName, id = gg.GroupFilterID, url = ConstructFilterIdUrl(ctx, gg.GroupFilterID)
        };
        if (gg.GroupsIds.ContainsKey(uid))
        {
            var groups = gg.GroupsIds[uid];
            if (groups.Count != 0)
            {
                ...
                foreach (var grp in groups)
                {
                    ...
                    if (v?.Art != null && v.Thumb != null)
                    {
                        ...
                        ob.art.thumb.Add(new Art { url = ConstructImageLinkFromRest(ctx, v.Thumb), index = 0 });
                        break;
                    }
                }
            }
    ...
    public static Filter FilterFromAnimeGroup(HttpContext ctx, ShokoGroup grp, int uid)
    {
        ...
        foreach (var ser in grp.Series.Randomize())
        {
            ...
            if (anim != null)
            {
                var fanart = anim.GetDefaultFanartDetailsNoBlanks();
                var banner = anim.GetDefaultWideBannerDetailsNoBlanks();
