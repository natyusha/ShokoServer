...
{
    [JsonIgnore]
    public static readonly Dictionary<string, string> SimplifiedSources = new()
        {
            {"raw", "unknown"},
            {"dtv", "tv"},
            {"hdtv", "tv"},
    ...
    public List<string> PreferredAudioCodecs { get; set; } = new()
        {
            "flac", "dca", "aac", "ac3", "wmav2", "wmapro", "adpcm_ms", "mp3", "mp2", "vp6f"
        };

    /// Preferred Resolutions, in order.
    public List<string> PreferredResolutions { get; set; } = new() { "2160p", "1440p", "1080p", "720p", "480p" };
    ...
    public List<string> PreferredSubGroups { get; set; } = new() { "fffpeeps", "doki", "commie", "horriblesubs" };

    /// Preferred Video Codecs, in order.
    /// Due to the complexity of Bit Depth, it has its own setting that is used when applicable.
    public List<string> PreferredVideoCodecs { get; set; } = new()
        {
            "av1", "hevc", "h264", "mpeg4", "vc1", "flv", "mpeg2", "mpeg1", "vp6f"
        };
    ...
    public FileQualityTypeListPair<List<string>> RequiredAudioCodecs { get; set; } =
        new FileQualityTypeListPair<List<string>>(new() { "flac", "dca", "aac" }, FileQualityFilterOperationType.IN);
    ...
    public FileQualityTypeListPair<List<string>> RequiredResolutions { get; set; } =
        new FileQualityTypeListPair<List<string>>(new() { "1080p" }, FileQualityFilterOperationType.GREATER_EQ);

    /// Required Source. Default must be BD or DVD release.
    /// Accepts IN, NOTIN
    public FileQualityTypeListPair<List<string>> RequiredSources { get; set; } =
        new FileQualityTypeListPair<List<string>>(new() { "bd", "dvd" }, FileQualityFilterOperationType.IN);
    ...
    public FileQualityTypeListPair<List<string>> RequiredSubGroups { get; set; } =
        new FileQualityTypeListPair<List<string>>(new() { "horriblesubs" }, FileQualityFilterOperationType.NOTIN);
    ...
    public FileQualityTypeListPair<List<string>> RequiredVideoCodecs { get; set; } =
        new FileQualityTypeListPair<List<string>>(new() { "hevc", "h264" }, FileQualityFilterOperationType.IN);
    ...
    [JsonIgnore] public List<string> _sources = new() { "bd", "dvd", "tv", "www", "unknown" };
