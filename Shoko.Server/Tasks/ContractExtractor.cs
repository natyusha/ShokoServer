...
internal class ContractExtractor
{
    ...
    public Stream GetContractsAsZipStream(ISessionWrapper session, string entityType)
    {
        ...
        using (var zip = new ZipArchive(buffer, ZipArchiveMode.Create, true))
        {
            ...
            {
                if (ContractExtractors.TryGetValue(entityType, out var contractDumpAction))
                {
                    contractDumpAction(session, zip);
                }
                else
        ...
        where TContract : class
    {
        ...
        {
            var zipEntryName = record[0].ToString();
            var contractLen = (int)record[1];
            var contractBinary = (byte[])record[2];

            if (contractLen > 0 && contractBinary != null)
            {
                var contract = CompressionHelper.DeserializeObject<TContract>(contractBinary, contractLen);

                lock (zipLock)
                {
                    var archiveEntry = zip.CreateEntry(zipEntryName, CompressionLevel.Optimal);
