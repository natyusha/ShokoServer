...
public class ShokoGroup_Filter
{
    ...
    internal List<ShokoGroup_FilterCondition> _conditions = new();

    public ShokoGroup_Filter Parent =>
        ParentGroupFilterID.HasValue ? RepoFactory.Shoko_Group_Filter.GetByID(ParentGroupFilterID.Value) : null;

    public ShokoGroup_Filter TopLevelGroupFilter
    {
        get
        {
            var parent = Parent;
    ...
    public virtual List<ShokoGroup_FilterCondition> Conditions
    {
        get
        {
            if (_conditions.Count == 0 && !string.IsNullOrEmpty(GroupConditions))
            {
                _conditions = JsonConvert.DeserializeObject<List<ShokoGroup_FilterCondition>>(GroupConditions);
            }
    ...
    public CL_GroupFilter ToClient()
    {
        var contract = new CL_GroupFilter
        {
            ...
            Childs = GroupFilterID == 0
                ? new HashSet<int>()
                : RepoFactory.Shoko_Group_Filter.GetByParentID(GroupFilterID).Select(a => a.GroupFilterID).ToHashSet()
        };
        return contract;
    }

    public static ShokoGroup_Filter FromClient(CL_GroupFilter gfc)
    {
    ...
    public CL_GroupFilterExtended ToClientExtended(Shoko_User user)
    {
    ...
    public bool UpdateGroupFilterFromSeries(CL_AnimeSeries_User ser, Shoko_User user)
    {
        if (ser == null)
        {
            return false;
        ...
        if (ApplyToSeries == 1)
        {
            result = CalculateGroupFilterSeries(RepoFactory.Shoko_Series.GetAll().Select(a => a.Id).ToHashSet(), ser, user);
            if (!result)
            {
                return false;
            }
            ...
            WriteLock(
                () =>
                {
                    GroupsIds[user?.JMMUserID ?? 0] = SeriesIds[user?.JMMUserID ?? 0]
                        .Select(a => RepoFactory.Shoko_Series.GetByID(a)?.TopLevelAnimeGroup?.Id ?? -1)
        ...
        else
        {
            ...
            var group = RepoFactory.Shoko_Group.GetByID(groupID.Value);
            ...
            result |= CalculateGroupFilterGroups(RepoFactory.Shoko_Group.GetAll().Select(a => a.Id).ToHashSet(), contract, user);
            if (!result)
            {
                return false;
            }
            ...
            WriteLock(
                () =>
                {
                    SeriesIds[user?.JMMUserID ?? 0] = GroupsIds[user?.JMMUserID ?? 0]
                        .SelectMany(
                            a =>
                                RepoFactory.Shoko_Group.GetByID(a)?.GetAllSeries()?.Select(b => b?.AnimeSeriesID ?? -1)
                        )
                        .Where(a => a != -1).ToHashSet();
                }
            );
    ...
    public bool UpdateGroupFilterFromGroup(CL_AnimeGroup_User grp, Shoko_User user)
    {
        if (grp == null)
        {
            return false;
        ...
        {
            result = false;
            var sers = new List<ShokoSeries>();
            ShokoGroup.GetAnimeSeriesRecursive(grp.AnimeGroupID, ref sers);
            ...
            foreach (var ser in sers)
            {
                ...
                result |= CalculateGroupFilterSeries(RepoFactory.Shoko_Series.GetAll().Select(a => a.Id).ToHashSet(), contract, user);
            }
        ...
        {
            result = CalculateGroupFilterGroups(RepoFactory.Shoko_Group.GetAll().Select(a => a.Id).ToHashSet(), grp, user);
            if (!result)
            {
                return false;
            }
            ...
            WriteLock(
                () =>
                {
                    SeriesIds[user?.JMMUserID ?? 0] = GroupsIds[user?.JMMUserID ?? 0].SelectMany(
                            a => RepoFactory.Shoko_Group.GetByID(a)
                                ?.GetAllSeries()
    ...
    private bool CalculateGroupFilterSeries(HashSet<int> allSeriesIds, CL_AnimeSeries_User ser, Shoko_User user)
    {
        if (ser == null)
        {
            return false;
    ...
    private bool CalculateGroupFilterGroups(HashSet<int> allGroupIds, CL_AnimeGroup_User grp, Shoko_User user)
    {
                ...
                () =>
                {
                    foreach (var user in SeriesIds.Keys)
                    {
                        GroupsIds[user] = SeriesIds[user].Select(
                                a =>
                                {
                                    var id = RepoFactory.Shoko_Series.GetByID(a)?.TopLevelAnimeGroup?.Id ?? -1;
                                    if (id == -1)
                                    {
                                        erroredSeries.Add(a);
                                    }
            ...
            foreach (var id in erroredSeries.OrderBy(a => a).ToList())
            {
                ...
                LogManager.GetCurrentClassLogger()
                    .Error("While calculating group filters, an AnimeSeries without a group was found: " +
                           (ser?.GetPreferredTitle() ?? id.ToString()));
            }
        }
        else
        {
                ...
                () =>
                {
                    foreach (var user in GroupsIds.Keys)
                    {
                        ...
                        SeriesIds[user] = ids.SelectMany(
                                a =>
                                    RepoFactory.Shoko_Group.GetByID(a)?.GetAllSeries()
                                        ?.Select(b => b?.AnimeSeriesID ?? -1)
                            )
                            .Where(a => a != -1).ToHashSet();
                    }
    ...
    private void EvaluateAnimeGroups()
    {
        ...
        var allGroupsIds = RepoFactory.Shoko_Group.GetAll().Select(a => a.Id).ToHashSet();
        foreach (var grp in RepoFactory.Shoko_Group.GetAllTopLevelGroups())
        {
            foreach (var user in users)
            {
                CalculateGroupFilterGroups(allGroupsIds, grp.GetUserContract(user.Id, cloned: false), user);
            }
        }
    }
    ...
    private void EvaluateAnimeSeries()
    {
        ...
        var allSeriesIds = allSeries.Select(a => a.Id).ToHashSet();
        foreach (var ser in allSeries)
        {
            ...
            foreach (var user in users)
            {
                CalculateGroupFilterSeries(allSeriesIds, ser.GetUserContract(user.Id, cloned: false), user);
            }
        }
    }
    ...
    public bool EvaluateGroupFilter(CL_AnimeGroup_User contractGroup, Shoko_User curUser)
    {
        ...
        {
            if (gfc.ConditionType != GroupFilterConditionType.AnimeGroup)
            {
                continue;
            }

            if (!int.TryParse(gfc.ConditionParameter, out var groupID) || groupID == 0)
            {
                break;
            }


            if (gfc.ConditionOperator== GroupFilterOperator.Equals &&
                groupID == contractGroup.AnimeGroupID)
            {
                return true;
            }

            if (gfc.ConditionOperator == GroupFilterOperator.NotEquals &&
                groupID != contractGroup.AnimeGroupID)
            {
                return true;
            }
    ...
    private bool EvaluateConditions(CL_AnimeGroup_User contractGroup, ShokoGroup_FilterCondition gfc)
    {
    ...
    public bool EvaluateGroupFilter(CL_AnimeSeries_User contractSerie, Shoko_User curUser)
    {
        //Directories don't count
        if ((FilterType & (int)GroupFilterType.Directory) == (int)GroupFilterType.Directory)
        {
    ...
    private bool EvaluateConditions(CL_AnimeSeries_User contractSerie, ShokoGroup_FilterCondition gfc)
    {
        var style = NumberStyles.Number;
        var culture = CultureInfo.InvariantCulture;
        ...
        switch (gfc.GetConditionTypeEnum())
        {
            ...
            case GroupFilterConditionType.AssignedTvDBInfo:
                ...
                var tvDBInfoMissing = !RepoFactory.CR_AniDB_TvDB.GetByAnimeID(contractSerie.AniDB_ID).Any();
            ...
            case GroupFilterConditionType.AssignedTvDBOrMovieDBInfo:
                ...
                var tvlinkMissing = !RepoFactory.CR_AniDB_TvDB.GetByAnimeID(contractSerie.AniDB_ID).Any();
    ...
    public bool Equals(ShokoGroup_Filter other)
    {
    ...
    public override bool Equals(object obj)
    {
        ...
        if (obj is ShokoGroup_Filter other)
            return Equals(other);
