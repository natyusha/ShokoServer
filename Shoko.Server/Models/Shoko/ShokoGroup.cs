...
public class ShokoGroup : IShokoGroup
{
    ...
    public ShokoGroup Parent => AnimeGroupParentID.HasValue
        ? RepoFactory.Shoko_Group.GetByID(AnimeGroupParentID.Value)
        : null;
    ...
    private ShokoGroup_User GetOrCreateUserRecord(int userid)
    {
        var rr = GetUserRecord(userid);
        if (rr != null)
        {
        ...
        rr = new ShokoGroup_User(userid, Id)
        {
            WatchedCount = 0,
        ...
        RepoFactory.Shoko_Group_User.Save(rr);
        return rr;
    }
    ...
    public ShokoGroup_User GetUserRecord(int userID)
    {
        return RepoFactory.Shoko_Group_User.GetByUserAndGroupIds(userID, Id);
    }
    ...
    public static void RenameAllGroups()
    {
        ...
        foreach (var grp in RepoFactory.Shoko_Group.GetAll())
        {
            ...
            if (series != null)
            {
                ...
                RepoFactory.Shoko_Group.Save(grp, true, false, false);
            }
        }
    ...
    public List<ShokoGroup> GetChildGroups()
    {
        return RepoFactory.Shoko_Group.GetByParentID(Id);
    }

    public List<ShokoGroup> GetAllChildGroups()
    {
        var grpList = new List<ShokoGroup>();
        GetAnimeGroupsRecursive(Id, ref grpList);
        return grpList;
    }
    ...
    /// </summary>
    public ShokoSeries GetMainSeries()
    {
        ShokoSeries series = null;

        // User overridden main series.
        if (DefaultAnimeSeriesID.HasValue)
        {
            series = RepoFactory.Shoko_Series.GetByID(DefaultAnimeSeriesID.Value);
            if (series != null)
            {
                return series;
            }
        ...
        {
            series = RepoFactory.Shoko_Series.GetByAnidbAnimeId(MainAniDBAnimeID.Value);
            if (series != null)
            {
                return series;
            }
    ...
    public void SetMainSeries(ShokoSeries series)
    {
        ...
        RepoFactory.Shoko_Group.Save(this, false, false);
    }

    public List<ShokoSeries> GetSeries()
    {
        var seriesList = RepoFactory.Shoko_Series.GetByGroupID(Id)
            .OrderBy(a => a.AirDate)
            .ToList();
        ...
        {
            ShokoSeries mainSeries = null;
            if (DefaultAnimeSeriesID.HasValue)
                mainSeries = seriesList.FirstOrDefault(ser => ser.Id == DefaultAnimeSeriesID.Value);
    ...
    public List<ShokoSeries> GetAllSeries(bool skipSorting = false)
    {
        var seriesList = new List<ShokoSeries>();
        GetAnimeSeriesRecursive(Id, ref seriesList);
        if (skipSorting)
            return seriesList;
    ...
    public static Dictionary<int, GroupVotes> BatchGetVotes(IReadOnlyCollection<ShokoGroup> animeGroups)
    {
        if (animeGroups == null)
        {
        ...
        var seriesByGroup = animeGroups.ToDictionary(g => g.Id, g => g.GetAllSeries());
        ...
        foreach (var animeGroup in animeGroups)
        {
            ...
            var permVoteCount = 0;
            var tempVoteCount = 0;
            var groupSeries = seriesByGroup[animeGroup.Id];
            ...
            votesByGroup[animeGroup.Id] = groupVotes;
        }
    ...
    public List<ITag> CustomTags
    {
        get
        {
            var tags = new List<Custom_Tag>();
            var tagIDs = new List<int>();


            // get a list of all the unique custom tags for all the series in this group
            foreach (var ser in GetAllSeries())
                foreach (var tag in RepoFactory.Custom_Tag.GetByAnimeID(ser.AniDB_ID))
                {
            ...
            return tags.OrderBy(a => a.Name).ToList();
        }
    }
    ...
    public List<ITitle> Titles
    {
        get
        {
            ...
            var animeTitles = new List<ITitle>();
    ...
    private void UpdateStats(bool watchedStats, bool missingEpsStats)
    {
        ...
        RepoFactory.Shoko_Group.Save(this, true, false);
        logger.Trace($"Finished Updating STATS for GROUP {GroupName} in {(DateTime.Now - start).TotalMilliseconds}ms");
    }

    /// <summary>
    /// Batch updates watched/missing episode stats for the specified sequence of <see cref="ShokoGroup"/>s.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    /// <param name="animeGroups">The sequence of <see cref="ShokoGroup"/>s whose missing episode stats are to be updated.</param>
    ...
    /// <param name="createdGroupUsers">The <see cref="ICollection{T}"/> to add any <see cref="ShokoGroup_User"/> records
    /// that were created when updating watched stats.</param>
    /// <param name="updatedGroupUsers">The <see cref="ICollection{T}"/> to add any <see cref="ShokoGroup_User"/> records
    ...
    public static void BatchUpdateStats(IEnumerable<ShokoGroup> animeGroups, bool watchedStats = true,
        bool missingEpsStats = true,
        ICollection<ShokoGroup_User> createdGroupUsers = null,
        ICollection<ShokoGroup_User> updatedGroupUsers = null)
    {
        if (animeGroups == null)
        {
            throw new ArgumentNullException(nameof(animeGroups));
        ...
        var allUsers =
            new Lazy<IReadOnlyList<Shoko_User>>(() => RepoFactory.Shoko_User.GetAll(), false);
    ...
    /// </summary>
    /// <param name="animeGroup">The <see cref="ShokoGroup"/> that is to have it's watched stats updated.</param>
    /// <param name="seriesList">The list of <see cref="ShokoSeries"/> that belong to <paramref name="animeGroup"/>.</param>
    /// <param name="allUsers">A sequence of all JMM users.</param>
    /// <param name="newAnimeGroupUsers">A methed that will be called for each processed <see cref="ShokoGroup_User"/>
    /// and whether or not the <see cref="ShokoGroup_User"/> is new.</param>
    private static void UpdateWatchedStats(ShokoGroup animeGroup,
        IReadOnlyCollection<ShokoSeries> seriesList,
        IEnumerable<Shoko_User> allUsers, Action<ShokoGroup_User, bool> newAnimeGroupUsers)
    {
        allUsers.ForEach(juser =>
        {
            ...
            if (userRecord == null)
            {
                userRecord = new ShokoGroup_User(juser.JMMUserID, animeGroup.Id);
                isNewRecord = true;
            }
    ...
    /// NOTE 2: Assumes that all the AnimeSeries have had their stats updated already.
    /// </remarks>
    /// <param name="animeGroup">The <see cref="ShokoGroup"/> that is to have it's missing episode stats updated.</param>
    /// <param name="seriesList">The list of <see cref="ShokoSeries"/> that belong to <paramref name="animeGroup"/>.</param>
    private static void UpdateMissingEpisodeStats(ShokoGroup animeGroup,
        IEnumerable<ShokoSeries> seriesList)
    {
    ...
    public static Dictionary<int, HashSet<GroupFilterConditionType>> BatchUpdateContracts(ISessionWrapper session,
        IReadOnlyCollection<ShokoGroup> animeGroups, bool updateStats)
    {
        if (session == null)
        {
        ...
        var allGroupIds = new Lazy<int[]>(
            () => animeGroups.Select(grp => grp.Id).ToArray(), false);
        var audioLangStatsByAnime = new Lazy<Dictionary<int, HashSet<string>>>(
            () => RepoFactory.CR_AniDB_File_Languages.GetLanguagesByAnime(allAnimeIds.Value), false);
        var subLangStatsByAnime = new Lazy<Dictionary<int, HashSet<string>>>(
            () => RepoFactory.CR_AniDB_File_Subtitles.GetLanguagesByAnime(allAnimeIds.Value), false);
        var tvDbXrefByAnime = new Lazy<ILookup<int, CR_AniDB_TvDB>>(
            () => RepoFactory.CR_AniDB_TvDB.GetByAnidbIds(allAnimeIds.Value), false);
        var traktXrefByAnime = new Lazy<ILookup<int, CrossRef_AniDB_TraktV2>>(
            () => RepoFactory.CR_AniDB_Trakt.GetByAnimeIDs(allAnimeIds.Value), false);
        ...
        var movieDbXRefByAnime = new Lazy<ILookup<int, CrossRef_AniDB_Other>>(
            () => RepoFactory.CR_AniDB_Other.GetByAnimeIDsAndType(session, allAnimeIds.Value,
                CrossRefType.MovieDB), false);
        var malXRefByAnime = new Lazy<ILookup<int, CrossRef_AniDB_MAL>>(
            () => RepoFactory.CR_AniDB_MAL.GetByAnimeIDs(session, allAnimeIds.Value), false);
        ...
        foreach (var animeGroup in animeGroups)
        {
            ...
            contract.AnimeGroupID = animeGroup.Id;
            ...
            var allSeriesForGroup = seriesByGroup[animeGroup.Id];
            ...
            if (localUpdateStats)
            {
                ...
                foreach (var series in allSeriesForGroup)
                {
                    ...
                    var vidsTemp = RepoFactory.Shoko_Video.GetByAniDBAnimeID(series.AniDB_ID);
                    var crossRefs =
                        RepoFactory.CR_Video_Episode.GetByAnidbAnimeId(series.AniDB_ID);
                    var crossRefsLookup = crossRefs.ToLookup(cr => cr.EpisodeID);
                    var dictVids = new Dictionary<string, Shoko_Video>();
                    ...
                    foreach (var ep in series.GetAnimeEpisodes())
                    {
                        ...
                        var epVids = new List<Shoko_Video>();
                ...
                contract.Stat_AnimeTypes = allSeriesForGroup
                    ...
                    .ToHashSet(StringComparer.InvariantCultureIgnoreCase);
                contract.Stat_AllVideoQuality = allVidQualByGroup.Value[animeGroup.Id]
                    .ToHashSet(StringComparer.InvariantCultureIgnoreCase);
                ...
                votesByGroup.TryGetValue(animeGroup.Id, out var votes);
            ...
            grpFilterCondTypesByGroup[animeGroup.Id] =
    ...
    public HashSet<GroupFilterConditionType> UpdateContract(ISessionWrapper session, bool updatestats)
    {
        ...
        return grpFilterCondTypesByGroup[Id];
    }

    private static ILookup<int, string> GetVideoQualities(IEnumerable<int> groupIds = null)
    {
        groupIds ??= RepoFactory.Shoko_Group.GetAll().Select(a => a.Id).ToList();
        return groupIds.SelectMany(id =>
            RepoFactory.Shoko_Series.GetByGroupID(id)
                .SelectMany(a =>
                    RepoFactory.CR_Video_Episode.GetByAnidbAnimeId(a.AniDB_ID)
    ...
    public void DeleteFromFilters()
    {
        foreach (var gf in RepoFactory.Shoko_Group_Filter.GetAll())
        {
            ...
            {
                if (gf.GroupsIds[k].Contains(Id))
                {
                    gf.GroupsIds[k].Remove(Id);
                    change = true;
                }
            }
    ...
    public static void GetAnimeGroupsRecursive(int animeGroupID, ref List<ShokoGroup> groupList)
    {
        ...
        {
            GetAnimeGroupsRecursive(childGroup.Id, ref groupList);
        }
    }

    public static void GetAnimeSeriesRecursive(int animeGroupID, ref List<ShokoSeries> seriesList)
    {
        var grp = RepoFactory.Shoko_Group.GetByID(animeGroupID);
        if (grp == null)
        {
        ...
        var thisSeries = grp.Series;
        seriesList.AddRange(thisSeries);

        foreach (var childGroup in grp.GetChildGroups())
        {
            GetAnimeSeriesRecursive(childGroup.Id, ref seriesList);
        }
    }
    ...
    public void DeleteGroup(bool updateParent = true)
    {
        ...
        var gfs =
            RepoFactory.Shoko_Group_Filter.GetWithConditionsTypes(new HashSet<GroupFilterConditionType>
            {
        ...
        foreach (var gf in gfs)
        {
            ...
            {
                RepoFactory.Shoko_Group_Filter.Delete(gf.GroupFilterID);
            }
            else
            {
        ...
        RepoFactory.Shoko_Group.Delete(this);
    ...
    string IShokoGroup.Name => GroupName;
    public IAnime MainSeries => GetMainSeries()?.GetAnime();

    IReadOnlyList<IAnime> IShokoGroup.Series => GetAllSeries().Select(a => a.GetAnime()).Where(a => a != null)
