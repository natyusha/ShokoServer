...
public class ShokoGroup_Repository : BaseCachedRepository<ShokoGroup, int>
{
    private static Logger logger = LogManager.GetCurrentClassLogger();

    private PocoIndex<int, ShokoGroup, int> Parents;

    private ChangeTracker<int> Changes = new();

    public ShokoGroup_Repository()
    {
        BeginDeleteCallback = cr =>
        {
            RepoFactory.Shoko_Group_User.Delete(RepoFactory.Shoko_Group_User.GetByGroupId(cr.Id));
    ...
    protected override int SelectKey(ShokoGroup entity)
    {
        return entity.Id;
    }
    ...
    {
        var grps = Cache.Values.Where(a => a.ContractVersion < ShokoGroup.CONTRACT_VERSION)
            .ToList();
        var max = grps.Count;
        var cnt = 0;
        ServerState.Instance.ServerStartingStatus = string.Format(Resources.Database_Validating,
            typeof(ShokoGroup).Name, " DbRegen");
        if (max <= 0)
        {
            return;
        }
        ...
        foreach (var g in grps)
        {
            ...
            if (cnt % 10 == 0)
            {
                ServerState.Instance.ServerStartingStatus = string.Format(
                    Resources.Database_Validating, typeof(ShokoGroup).Name,
                    " DbRegen - " + cnt + "/" + max);
            }
        }

        ServerState.Instance.ServerStartingStatus = string.Format(Resources.Database_Validating,
            typeof(ShokoGroup).Name,
            " DbRegen - " + max + "/" + max);
    }

    public override void Save(ShokoGroup obj)
    {
        Save(obj, true, true);
    }

    public void Save(ShokoGroup grp, bool updategrpcontractstats, bool recursive,
        bool verifylockedFilters = true)
    {
        ...
        Changes.AddOrUpdate(grp.Id);

        if (verifylockedFilters)
        {
            RepoFactory.Shoko_Group_Filter.CreateOrVerifyDirectoryFilters(false, grp.Contract?.Stat_AllTags,
        ...
        if (grp.AnimeGroupParentID.HasValue && recursive)
        {
            ...
            if (pgroup != null && pgroup.AnimeGroupParentID == grp.Id)
            {
    ...
    public void InsertBatch(ISessionWrapper session, IReadOnlyCollection<ShokoGroup> groups)
    {
        if (session == null)
        {
        ...
        Changes.AddOrUpdateRange(groups.Select(g => g.Id));
    }

    public void UpdateBatch(ISessionWrapper session, IReadOnlyCollection<ShokoGroup> groups)
    {
        if (session == null)
        {
            throw new ArgumentNullException(nameof(session));
    ...
    public void DeleteAll(ISessionWrapper session, int? excludeGroupId = null)
    {
        ...
        lock (GlobalDBLock)
        {
            ...
            {
                session.CreateQuery("delete ShokoGroup ag where ag.id <> :excludeId")
                    .SetInt32("excludeId", excludeGroupId.Value)
                    .ExecuteUpdate();
            }
            else
            {
                session.CreateQuery("delete ShokoGroup ag")
                    .ExecuteUpdate();
            }
        }
        ...
        if (excludeGroupId != null)
        {
            Changes.RemoveRange(allGrps.Select(g => g.Id)
        ...
        if (excludeGroupId != null)
        {
            var excludedGroup = allGrps.FirstOrDefault(g => g.Id == excludeGroupId.Value);
    ...
    public List<ShokoGroup> GetByParentID(int parentid)
    {
        return ReadLock(() => Parents.GetMultiple(parentid));
    }

    public List<ShokoGroup> GetAllTopLevelGroups()
    {
