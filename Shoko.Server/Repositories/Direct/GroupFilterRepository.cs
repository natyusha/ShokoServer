...
public class GroupFilterRepository : BaseCachedRepository<ShokoGroup_Filter, int>
{
    private PocoIndex<int, ShokoGroup_Filter, int> Parents;

    private BiDictionaryManyToMany<int, GroupFilterConditionType> Types;

    private readonly ChangeTracker<int> Changes = new();

    public List<ShokoGroup_Filter> PostProcessFilters { get; set; }
    ...
    protected override int SelectKey(ShokoGroup_Filter entity)
    {
    ...
    public override void PopulateIndexes()
    {
        ...
        PostProcessFilters = new List<ShokoGroup_Filter>();
    }

    public override void RegenerateDb()
    {
        foreach (var g in Cache.Values.Where(g =>
                     (g.GroupFilterID != 0 && g.GroupsIdsVersion < ShokoGroup_Filter.GROUPFILTER_VERSION) ||
                     g.GroupConditionsVersion < ShokoGroup_Filter.GROUPCONDITIONS_VERSION).ToList())
        {
    ...
    public override void PostProcess()
    {
        ...
        {
            if (g.GroupsIdsVersion < ShokoGroup_Filter.GROUPFILTER_VERSION ||
                g.GroupConditionsVersion < ShokoGroup_Filter.GROUPCONDITIONS_VERSION ||
                g.SeriesIdsVersion < ShokoGroup_Filter.SERIEFILTER_VERSION)
            {
        ...
        foreach (var gf in PostProcessFilters)
        {
            ...
            ServerState.Instance.ServerStartingStatus = string.Format(
                ...
                gf.GroupFilterName);
            if (gf.GroupsIdsVersion < ShokoGroup_Filter.GROUPFILTER_VERSION ||
                gf.GroupConditionsVersion < ShokoGroup_Filter.GROUPCONDITIONS_VERSION ||
                gf.SeriesIdsVersion < ShokoGroup_Filter.SERIEFILTER_VERSION)
            {
        ...
        var all = GetAll();
        var set = new HashSet<ShokoGroup_Filter>(all);
    ...
    public void CleanUpEmptyDirectoryFilters()
    {
        var toremove = GetAll().Where(a => (a.FilterType & (int)GroupFilterType.Directory) != 0)
            .Where(gf => gf.GroupsIdsVersion == ShokoGroup_Filter.GROUPFILTER_VERSION &&
                         gf.SeriesIdsVersion == ShokoGroup_Filter.SERIEFILTER_VERSION && gf.GroupsIds.Count == 0 &&
    ...
    public void CreateOrVerifyLockedFilters()
    {
        ...
        else if (cwatching == null)
        {
            var gf = new ShokoGroup_Filter
            {
                ...
                Conditions = new List<ShokoGroup_FilterCondition>()
            };
            var gfc = new ShokoGroup_FilterCondition
            {
                ConditionType = GroupFilterConditionType.HasWatchedEpisodes,
                ConditionOperator = GroupFilterOperator.Include,
            };
            gf.Conditions.Add(gfc);
            gfc = new ShokoGroup_FilterCondition
            {
                ConditionType = GroupFilterConditionType.HasUnwatchedEpisodes,
                ConditionOperator = GroupFilterOperator.Include,
            };
            gf.Conditions.Add(gfc);
            gf.CalculateGroupsAndSeries();
        ...
        if (tagsdirec == null)
        {
            tagsdirec = new ShokoGroup_Filter
            {
        ...
        if (yearsdirec == null)
        {
            yearsdirec = new ShokoGroup_Filter
            {
        ...
        {
            seasonsdirec = new ShokoGroup_Filter
            {
        ...
        HashSet<int> airdate = null, SortedSet<string> seasons = null)
    {
        ...
        if (tagsdirec != null)
        {
            ...
            foreach (var s in alltags)
            {
                ...
                var gfc = new ShokoGroup_FilterCondition
                {
                    ConditionType = GroupFilterConditionType.Tag,
                    ConditionOperator = GroupFilterOperator.In,
        ...
        if (yearsdirec != null)
        {
            ...
            foreach (var s in allyears)
            {
                ...
                var gfc = new ShokoGroup_FilterCondition
                {
                    ConditionType = GroupFilterConditionType.Year,
                    ConditionOperator = GroupFilterOperator.Include,
                    ConditionParameter = s,
                };
                yf.Conditions.Add(gfc);
        ...
        if (seasonsdirectory != null)
        {
            ...
            {
                var grps =
                    RepoFactory.Shoko_Series.GetAll().ToList();
            ...
            foreach (var season in allseasons)
            {
                ...
                var gfc = new ShokoGroup_FilterCondition
                {
                    ConditionType = GroupFilterConditionType.Season,
                    ConditionOperator = GroupFilterOperator.In,
    ...
    public override void Save(ShokoGroup_Filter obj)
    {
        Save(obj, false);
    }

    public void Save(ShokoGroup_Filter obj, bool onlyconditions)
    {
        WriteLock(
            () =>
            {
                ...
                obj.GroupConditionsVersion = ShokoGroup_Filter.GROUPCONDITIONS_VERSION;
                base.Save(obj);
                if (resaveConditions)
                {
                    Save(obj, true);
    ...
    public override void Save(IReadOnlyCollection<ShokoGroup_Filter> objs)
    {
    ...
    public override void Delete(IReadOnlyCollection<ShokoGroup_Filter> objs)
    {
    ...
    /// <summary>
    /// Updates a batch of <see cref="ShokoGroup_Filter"/>s.
    /// </summary>
    /// <remarks>
    /// This method ONLY updates existing <see cref="ShokoGroup_Filter"/>s. It will not insert any that don't already exist.
    /// </remarks>
    /// <param name="session">The NHibernate session.</param>
    /// <param name="groupFilters">The batch of <see cref="ShokoGroup_Filter"/>s to update.</param>
    /// <exception cref="ArgumentNullException"><paramref name="session"/> or <paramref name="groupFilters"/> is <c>null</c>.</exception>
    public void BatchUpdate(ISessionWrapper session, IEnumerable<ShokoGroup_Filter> groupFilters)
    {
        if (session == null)
        {
    ...
    public List<ShokoGroup_Filter> GetByParentID(int parentid)
    {
        return ReadLock(() => Parents.GetMultiple(parentid));
    }

    public List<ShokoGroup_Filter> GetTopLevel()
    {
    ...
    public void CalculateAnimeSeriesPerTagGroupFilter(ISessionWrapper session)
    {
        ...
        var users = new List<Shoko_User> { null };
        users.AddRange(RepoFactory.Shoko_User.GetAll());
        ...
        Parallel.ForEach(tags, tag =>
        {
            foreach (var series in tag.ToList().SelectMany(a => RepoFactory.AniDB_Anime_Tag.GetAnimeWithTag(a.TagID)))
            {
                ...
                foreach (var user in users)
                {
                    ...
                    if (somethingDictionary.ContainsKey(user?.Id ?? 0))
                    {
                        if (somethingDictionary[user?.Id ?? 0].ContainsKey(tag.Key))
                        {
                            somethingDictionary[user?.Id ?? 0][tag.Key]
                                .Add(series.AnimeSeriesID);
                        }
                        else
                        {
                            somethingDictionary[user?.Id ?? 0].AddOrUpdate(tag.Key,
                    ...
                    {
                        somethingDictionary.AddOrUpdate(user?.Id ?? 0, id =>
                            {
    ...
    private void DropAllTagFilters(ISessionWrapper session)
    {
        ...
        lock (GlobalDBLock)
        {
            ...
            session.CreateQuery($"DELETE FROM {nameof(ShokoGroup_Filter)} WHERE FilterType = {(int)GroupFilterType.Tag};")
                .ExecuteUpdate();
            trans.Commit();
        }
    }

    private void CreateAllTagFilters(ISessionWrapper session, ShokoGroup_Filter tagsdirec,
        Dictionary<int, ILookup<string, int>> lookup)
    {
        ...
        var alltags = new HashSet<string>(
            RepoFactory.AniDB_Tag.GetAllForLocalSeries().Select(a => a.TagName.Replace('`', '\'')),
            StringComparer.InvariantCultureIgnoreCase);
        var toAdd = new List<ShokoGroup_Filter>(alltags.Count);

        var users = RepoFactory.Shoko_User.GetAll().ToList();
        ...
        foreach (var s in alltags)
        {
            ...
            yf.GroupsIds[0] = yf.SeriesIds[0]
                .Select(id => RepoFactory.Shoko_Series.GetByID(id).TopLevelAnimeGroup?.Id ?? -1)
            ...
            foreach (var user in users)
            {
                ...
                if (!lookup.ContainsKey(user.Id) || !lookup[user.Id].Contains(key))
                {
                    continue;
                }

                yf.SeriesIds[user.Id] = lookup[user.Id][key].ToHashSet();
                yf.GroupsIds[user.Id] = yf.SeriesIds[user.Id]
                    .Select(id => RepoFactory.Shoko_Series.GetByID(id).TopLevelAnimeGroup?.Id ?? -1)
                    .Where(id => id != -1).ToHashSet();
            }
            ...
            yf.GroupConditionsVersion = ShokoGroup_Filter.GROUPCONDITIONS_VERSION;
            toAdd.Add(yf);
        }
    ...
    public List<ShokoGroup_Filter> GetLockedGroupFilters()
    {
        return ReadLock(() => Cache.Values.Where(a => a.Locked == 1).ToList());
    }

    public List<ShokoGroup_Filter> GetWithConditionTypesAndAll(HashSet<GroupFilterConditionType> types)
    {
        return ReadLock(
            () =>
            {
    ...
    public List<ShokoGroup_Filter> GetWithConditionsTypes(HashSet<GroupFilterConditionType> types)
    {
        return ReadLock(
            () =>
            {
