...
public class AniDB_Anime_TagRepository : BaseCachedRepository<AniDB_Anime_Tag, int>
{
    ...
    public List<ShokoSeries> GetAnimeWithTag(string tagName)
    {
        return GetAll().AsParallel()
            .Where(a => RepoFactory.AniDB_Tag.GetByName(tagName).Select(b => b.TagID).Contains(a.TagID))
            .Select(a => RepoFactory.Shoko_Series.GetByAnidbAnimeId(a.AnimeID))
            .Where(a => a != null)
            .ToList();
    }

    public List<ShokoSeries> GetAnimeWithTag(int tagID)
    {
        return GetByTagID(tagID).Select(a => RepoFactory.Shoko_Series.GetByAnidbAnimeId(a.AnimeID))
